...
def generator(inputs, dropout_rate=None, activation_fn=tf.nn.leaky_relu, normalizer_fn=slim.batch_norm,
    ...
    with tf.variable_scope(scope, "generator", [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.dropout, slim.batch_norm], is_training=is_training), \
            ...
            net = fully_connected(inputs=net, num_outputs=1024, scope="fully_connected_1")

            net = fully_connected(inputs=net, num_outputs=2 * 2 * 1024, scope="fully_connected_2")

            net = tf.reshape(net, shape=[-1, 2, 2, 1024], name="reshape")

            net = deconv(inputs=net, num_filters=512, kernel_size=4, stride=2, scope="deconv_1")

            net = deconv(inputs=net, num_filters=256, kernel_size=4, stride=2, scope="deconv_2")

            net = deconv(inputs=net, num_filters=128, kernel_size=4, stride=2, scope="deconv_3")

            net = deconv(inputs=net, num_filters=64, kernel_size=4, stride=2, scope="deconv_4")

            net = deconv(inputs=net, num_filters=3, kernel_size=4, stride=2, scope="deconv_5",
...
def discriminator(inputs, dropout_rate=None, activation_fn=tf.nn.leaky_relu, normalizer_fn=slim.batch_norm,
                  is_training=True, reuse=None, scope=None):
    with tf.variable_scope(scope, "discriminator", [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.dropout, slim.batch_norm], is_training=is_training), \
            ...
            net = conv(inputs=net, num_filters=64, kernel_size=3, stride=2, scope="conv_1")

            net = conv(inputs=net, num_filters=128, kernel_size=3, stride=2, scope="conv_2")

            net = conv(inputs=net, num_filters=256, kernel_size=3, stride=2, scope="conv_3")

            net = conv(inputs=net, num_filters=512, kernel_size=3, stride=2, scope="conv_4")

            net = conv(inputs=net, num_filters=1024, kernel_size=3, stride=2, scope="conv_5")

            net = slim.flatten(net)

            net = fully_connected(inputs=net, num_outputs=1024, scope="fully_connected_1")

            net = fully_connected(inputs=net, num_outputs=1, scope="fully_connected_2", activation_fn=None)
